...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 1. Temukan elemen H1 dan simpan teksnya
    const textElement = document.querySelector('.tulisan');
    const fullText = textElement.innerText;
    
    // 2. Kosongkan teks aslinya di halaman
    textElement.innerText = '';

    let charIndex = 0;
    const typingSpeed = 150; // Kecepatan ketik dalam milidetik (ubah angkanya)

    function typeWriter() {
        // Jika masih ada huruf yang belum diketik
        if (charIndex < fullText.length) {
            // Tambahkan satu huruf ke H1
            textElement.innerText += fullText.charAt(charIndex);
            charIndex++;
            
            // Panggil fungsi ini lagi setelah jeda (typingSpeed)
            setTimeout(typeWriter, typingSpeed);
        } else {
            // Jika sudah selesai mengetik, tambahkan kelas
            // Ini agar kita bisa menghilangkan kursor (lihat CSS)
            textElement.classList.add('selesai-ketik');
            const nextButton = document.getElementById('nextButton');
            nextButton.classList.add('show');
        }
    }

    // 3. Mulai efek mengetiknya
    typeWriter();
});